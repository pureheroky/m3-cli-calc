...
import type { Expr } from "../src/core/ast.js";
...
describe("arithmetic plugin (smoke)", () => {
  ...
  it("adds reals", () => {
    const reg = makeReg();
    const expr: Expr = Bin("+", Lit(R(1)), Lit(R(2)));
  ...
  it("adds fractions", () => {
    const reg = makeReg();
    const expr: Expr = Bin("+", Lit(F(1n, 3n)), Lit(F(1n, 6n)));
    const v = evaluate(expr, reg);
    // 1/3 + 1/6 = 1/2
  ...
  it("multiplies complex", () => {
    const reg = makeReg();
    const expr: Expr = Bin("*", Lit(C(1, 2)), Lit(C(3, -1)));
    const v = evaluate(expr, reg);
    // (1+2i)*(3-i) = 5+5i
  ...
  it("powers bigints", () => {
    const reg = makeReg();
    const expr: Expr = Bin("^", Lit(B(2n)), Lit(B(3n)));
    const v = evaluate(expr, reg);
